...
export class NodeHelper {
  ...
  static _emptyInvalidNodesList() {
    ...
    if (
      EnvironmentHelper.getFallbackURIs().length === 0 &&
      Object.keys(NodeHelper.currentRemovedNodes).length === EnvironmentHelper.getAPIUris().length
    ) {
   ...
   */
  static getNodesUris = () => {
    let allURIs = EnvironmentHelper.getAPIUris();
    ...
    if (allURIs.length === 0) {
      ...
      allURIs = EnvironmentHelper.getFallbackURIs();
    }
    return allURIs;
  };
